...
class LabeledDatasetCreator:
    ...
    def create_unique_dataset(self, labeled_dataset_texts):
        ...
        # Preserve order while efficiently filtering
        metadata_rel = [meta for i, meta in enumerate(self.metadata) if i in labeled_indices_set]
        print(f"Number of labeled metadata: {len(metadata_rel)}")
        
        metadata_rel_unique = [meta for i, meta in enumerate(self.metadata) if i in labeled_indices_unique_set]
        print(f"Number of labeled metadata (unique): {len(metadata_rel_unique)}")
        ...
        return {
            ...
            "metadata_unique": metadata_rel_unique, 
    ...
    def run(self, min_topic_words:int = 2, max_words_other_topics:int = 0, exact_match:bool = False):
        ...
        print(f"Number of corresponding metadata: {len(results["metadata"])}\n")
        ...
        print(f"Number of corresponding metadata: {len(results["metadata_unique"])}\n")
