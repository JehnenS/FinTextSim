...
class TopicAssignmentEvaluator:
    """
    assess the assignments of topics to sentences across multiple BERTopic and classical models
    """
    def __init__(self, 
                 texts, 
                 bertopic_model_list, 
                 classical_model_list,
                 main_topics_classical,
                 subplot_titles, 
                 keywords, 
                 num_words:int = 5):
        
        self.num_cols = len(bertopic_model_list) + len(classical_model_list)
        self.texts = texts
        self.bertopic_model_list = bertopic_model_list
        self.classical_model_list = classical_model_list
        self.main_topics_classical = main_topics_classical
        self.subplot_titles = subplot_titles
        self.keywords = keywords
        self.num_words = num_words

    def _extract_topic_words_classical_(self, model):
        """
        Extract the top-n words per topic from the classical model
        """
        topics = model.show_topics(
            num_topics = -1, #ensure that all topics are extracted
            num_words = self.num_words, #set number of top-words to extract
            formatted = False
        )
        
        #extract words and betas separately
        topic_words = [[word for word, beta in word_prob_list] for _, word_prob_list in topics]
        betas = [[beta for word, beta in word_prob_list] for _, word_prob_list in topics]
        return topic_words
    ...
    def plot_wordclouds(self, doc_num, plot_name=None):
        ...
        word_list = []
        #extract the topic representations for bertopic models
        for model in self.bertopic_model_list:
        ...
        #add the classical topic representations
        for main_topic, model in zip(self.main_topics_classical, self.classical_model_list):
            topic = main_topic[doc_num] #extract main topic from the classical models
            topic_reps = self._extract_topic_words_classical_(model) #extract all topic representations from the model
            topic_rep = topic_reps[topic]
            lemmatized_word_list.append(topic_rep)
