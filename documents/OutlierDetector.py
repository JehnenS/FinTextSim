...
class OutlierDetector:
    def __init__(self,
                text_list:list,
                metadata_list:list,
                min_words_threshold:int = 250,
                 z_score_threshold:float = 2.0,
                 output_dir:str = "paper1/Data/10-K/item7/item7_text_outlier.pkl"
                ):
    ...
    def filter_min_words(self):
        ...
        filtered_text_metadata_list = [(text, meta) for text, meta, num_words in tqdm(zip(self.text_list, self.metadata_list, number_of_words), desc = "Filter by min. words") if num_words >= self.min_words_threshold]
        filtered_text_list, filtered_metadata_list = zip(*filtered_text_metadata_list)
     
        print(f"docs at beginning/before removing by min_chars: {len(self.text_list)}")
        print(f"docs filtered by min_words (n = {self.min_words_threshold}): {len(self.text_list) - len(filtered_text_list)}")
        print(f"docs remaining after removing min_words (n = {self.min_words_threshold}): {len(filtered_text_list)}")
    ...
    def filter_zscores(self, z_scores):
        """
        ...
        filtered_text_metadata_z_score_list = [(text, meta, z_score) for text, meta, z_score in tqdm(zip(self.text_list, self.metadata_list, z_scores), desc = "z-score filtering") if z_score <= self.z_score_threshold]
    ...
    def run(self):
        ...
        _, number_of_words_post = self.filter_min_words()
        ...
        self.filter_zscores(z_scores)

        #5. Save results
        self.save_results(self.text_list, self.metadata_list)