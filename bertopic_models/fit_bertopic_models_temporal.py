...
rel_models = ["htl_temporal"]

#----------Load embeddings

embedding_loc = "paper1/Data/topic_model_input/embeddings_2016_2023_temporal.pkl"
...
config_save_loc = "paper1/Topic_Models/BERTopic/config_temporal.pkl"
##############
...
for model in embedding_data.keys():
    if model not in rel_models:
        continue
    #extract embeddings
    embeddings = embedding_data[model]
    print(f"Model name: {model}")
    print(f"Shape of embeddings: {embeddings.shape}")
    model_loc = f"paper1/Topic_Models/BERTopic/bertopic_{model}_v2"
    ...
    bertopic_model = BERTopic(
        ...
        calculate_probabilities = False, #set to False to increase the speed --> probs can be approximated later on
