...
def generate_embeddings_classifier(model, tokenizer, texts, layer = "roberta"):
    """
    Generate embeddings with classifier model
    """
    ...
    for text in tqdm(texts, desc = "Progress"):   
        inputs = tokenizer(
            text,
            return_tensors="pt",
            truncation=True,
        ...
        with torch.no_grad():
            #outputs = f"model.{layer}(**inputs)"
            outputs = getattr(model, layer)(**inputs)
        
        #extract token-level embeddings from last hidden layer
