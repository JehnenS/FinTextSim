...
class Classifier:
    ...
    def evaluate_cv_lr(self, X, y, n_splits=5):
        """
        Evaluate Logistic Regression using k-fold cross-validation (for model evaluation).
        Computes ACC, AUC, LogLoss, and F1-Score for each fold.
        """
        ...
        print(f"\nStarting {n_splits}-fold cross-validation for evaluation")
        
        #ensure CPU-based arrays for StratifiedKFold
        y_cpu = y.to_pandas() if hasattr(y, "to_pandas") else cp.asnumpy(y)
        X_cpu = X.to_pandas() if hasattr(X, "to_pandas") else cp.asnumpy(X)
        
        skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
         
        accs, aucs, f1s = [], [], []
        accs_filtered, aucs_filtered, f1s_filtered = [], [], []
        
        #iterate over each fold
        for fold, (train_idx, val_idx) in tqdm(enumerate(skf.split(X_cpu, y_cpu)), desc = "Fold Progress"):
            X_train = X.iloc[train_idx]
            X_val = X.iloc[val_idx]
            y_train = y.iloc[train_idx]
            y_val = y.iloc[val_idx]
        
            model = LogisticRegression(max_iter=1000, C=1.0, penalty="l2", tol=1e-4, class_weight="balanced")
            model.fit(X_train, y_train)
        
            y_val_pred = model.predict(X_val)
            y_val_prob = model.predict_proba(X_val).iloc[:, 1]

            print(f"\n--- Fold {fold} ---")
            
            eval_full, eval_filtered  = self.evaluate_model(
                y_true = y_val,
                y_pred = y_val_pred,
                y_prob = y_val_prob,
                n_bootstrap_auc = 0
            ...
            auc = eval_full["auc"]
            f1 = eval_full["f1"]
            precision = eval_full["precision"]
            recall = eval_full["recall"]
        
            accs.append(float(cp.asnumpy(acc) if isinstance(acc, cp.ndarray) else acc))
            aucs.append(float(cp.asnumpy(auc) if isinstance(auc, cp.ndarray) else auc))
            f1s.append(float(cp.asnumpy(f1) if isinstance(f1, cp.ndarray) else f1))

            acc_filtered = eval_filtered["acc"]
            auc_filtered = eval_filtered["auc"]
            f1_filtered = eval_filtered["f1"]
            precision_filtered = eval_filtered["precision"]
            recall_filtered = eval_filtered["recall"]
        
            accs_filtered.append(float(cp.asnumpy(acc_filtered) if isinstance(acc_filtered, cp.ndarray) else acc_filtered))
            aucs_filtered.append(float(cp.asnumpy(auc_filtered) if isinstance(auc_filtered, cp.ndarray) else auc_filtered))
            f1s_filtered.append(float(cp.asnumpy(f1_filtered) if isinstance(f1_filtered, cp.ndarray) else f1_filtered))
        ...
        results_df = pd.DataFrame({
            ...
            "auc": aucs,
            "f1": f1s,
            "accuracy_filtered": accs_filtered,
            "auc_filtered": aucs_filtered,
            "f1_filtered": f1s_filtered
        })
        
        print("\nCross-validation summary:")
        print(results_df.mean().to_frame("mean").join(results_df.std().to_frame("std")))
    ...
    def evaluate_model(
        ...
        def compute_metrics(y_true, y_pred, y_prob, label="Full dataset", n_booststrap_auc = n_bootstrap_auc):
            ...
            acc_best = (tp_b + tn_b) / (tn_b + fp_b + fn_b + tp_b)
            precision_best = tp_b / (tp_b + fp_b) if (tp_b + fp_b) > 0 else 0
            ...
            print(f"Metrics at best threshold: ACC={acc_best:.3f}, PREC={precision_best:.3f}, REC={recall_best:.3f}")
