...
result_loc = load_result_loc(results_dict, model_name = "xgb", base_path = "paper1/Results/ML/Financials")
...
loc_fts = "paper1/Data/Features/text_features/text_features_fintextsim_htl.csv"
loc_am = "paper1/Data/Features/text_features/text_features_am.csv"
loc_mpnet = "paper1/Data/Features/text_features/text_features_mpnet.csv"
...
df_fin_only, df_fin_fts, df_fin_am, df_fin_mpnet, df_fin_lda_tf, df_fin_lda_tfidf, df_fin_nmf_tf, df_fin_nmf_tfidf = builder.build_all()

dfs = [df_fin_only, df_fin_fts, df_fin_am, df_fin_mpnet, df_fin_lda_tf, df_fin_lda_tfidf, df_fin_nmf_tf, df_fin_nmf_tfidf] #set df_fin_text as base for text only df --> drop same rows based on financial features; then filter columns in X_train and X_test for sentiment features
df_names = ["fin", "fin+fts", "fin+am", "fin+mpnet", "fin+lda+tf", "fin+lda+tfidf", "fin+nmf+tf", "fin+nmf+tfidf"]  
...
for i, df in enumerate(dfs):
    ...
    y_train_pred, y_test_pred, y_train_prob, y_test_prob = classifier.xgb_classification(
        ...
        subsample = 0.5, #0.5 in Chen et al. 2022
    ...
    #cv_results = classifier.evaluate_cv_lr()
