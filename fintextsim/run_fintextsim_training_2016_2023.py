"""
Train FinTextSim with AdaptiveCircleLoss
"""
...
model_dir = "paper1/Fintextsim_Models/fintextsim_2016_2023"
embedding_dir = "paper1/Results/Embeddings/test_data/fintextsim_2016_2023.pkl"

n_epochs = results_dict["config"].get("n_epochs")
transition_steps = results_dict["config"].get("transition_steps")

initial_margin = results_dict["config"].get("initial_margin")
initial_scale = results_dict["config"].get("initial_scale")
final_margin = results_dict["config"].get("final_margin")
final_scale = results_dict["config"].get("final_scale")
batch_size = results_dict["config"].get("batch_size")

print(f"\nInitial margin: {initial_margin}")
print(f"Initial scale: {initial_scale}")
print(f"Final margin: {final_margin}")
print(f"Final scale: {final_scale}")
print(f"Switching to hardest phase at {transition_steps} forward passes\n")
...
file_loc = "paper1/Data/train_test_sets/train_test_sets_2016_2023.pkl"
...
base_models = [modern_bert_base_model, finbert_base_model, bert_base_model]
base_model_names = ["modern_bert", "finbert", "BERT"]
...
for i, model in tqdm(enumerate(base_models), desc = "Model progress"):
    ...
    print(f"Batch size: {batch_size}; \ninitial nmargin: {initial_margin}\nfinal margin: {final_margin}\ninitial_scale: {initial_scale}\nfinal scale: {final_scale}\nepochs: {n_epochs}")
    
    train_loss_1 = CircleLossText(model = model, margin = initial_margin, min_margin = final_margin, scale = initial_scale, max_scale = final_scale, debug = False, max_forward_passes = transition_steps)
