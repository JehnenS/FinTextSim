...
#-------------BERTopic models --> iteratre over all .npy files
topic_vector_dir = "paper1/Results/BERTopic_Models"
listdir = os.listdir(topic_vector_dir)
npy_files = [file for file in listdir if file.endswith(".npy")]
model_names = [name.split("_topic_distributions.npy")[0] for name in npy_files]
npy_file_locs = [os.path.join(topic_vector_dir, file) for file in npy_files]
...
for name, vector in zip(model_names, npy_file_locs):
    print(f"\nExtracting topic-distributions and aggregating them into document-topic distributions for {name}")
    result_loc = f"paper1/Data/Features/text_features/text_features_{name}_temporal.csv"
    print(f"Results will be saved to {result_loc}")

    topic_vector = np.load(vector)

    doc_vectors = creator.run_bertopic(topic_vector, meta, cik_to_ticker)
